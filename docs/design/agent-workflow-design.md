...
| 数据主要放 JSONB，已有节点 attempt 表及唯一索引 | [初始表](../../server/migrations/502_workflow.up.sql)、[attempt 索引](../../server/migrations/516_workflow_node_attempt.up.sql) | 可编辑图保留 JSON，运行控制字段和执行实例关系化 |
